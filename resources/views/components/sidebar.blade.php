<div class="relative" x-data x-cloak>
    ...
    <div class="flex h-full overflow-hidden">
        ...
        <button 
            class="md:hidden fixed top-4 right-4 z-30 p-3 text-white shadow-lg active:scale-95 transition-all" 
    ...
    <aside x-cloak
        :class="$store.sidebar.isOpen ? 'w-72 translate-x-0' : '-translate-x-full md:translate-x-0 md:w-24'"
        ...
        <div class="mb-8 relative z-10">
            <div class="flex justify-end md:hidden mb-4">
                <button @click="$store.sidebar.close()" class="p-2 text-slate-500 hover:text-white transition-colors">
            ...
            <div class="flex items-center justify-between gap-3">
                ...
                <button @click="$store.sidebar.toggle()"
                    ...
                    <svg class="w-4 h-4 transition-transform duration-500" :class="{'rotate-180': !$store.sidebar.isOpen}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div x-show="$store.sidebar.isOpen" x-transition.opacity class="mb-4 px-4 relative z-10 hidden md:block">
        ...
        <nav class="space-y-1.5 relative z-10 flex-1 overflow-y-auto custom-scroll pr-1">
            @php
                $menus = [
                    ['name' => 'User Management', 'route' => 'admin.users.index', 'icon' => 'M16 11a4 4 0 10-8 0 4 4 0 008 0z M2 20a10 10 0 0120 0'], 
                    ['name' => 'Kapster', 'route' => 'admin.kapsters.index', 'icon' => 'M12 8v4l3 3'], 
                    ['name' => 'Kapster Shifts', 'route' => 'admin.kapster_shifts.index', 'icon' => 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z'],
                    ['name' => 'Service Menu', 'route' => 'admin.services.index', 'icon' => 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2H9z'],
                    ...
                    ['name' => 'Operating Report', 'route' => 'admin.reports.index', 'icon' => 'M9 17v-2m3 2v-4m3 4v-6M7 21h10a2 2 0 002-2V7a2 2 0 00-2-2H7a2 2 0 00-2 2v12a2 2 0 002 2z'],
                ];
            @endphp
            ...
            @foreach($menus as $menu)
                ...
                <a href="{{ route($menu['route']) }}" 
                   class="group flex items-center p-3 rounded-2xl transition-all duration-300 {{ $isActive ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'text-gray-500 hover:bg-white/[0.03] hover:text-white' }}">
                    ...
                    <span x-show="$store.sidebar.isOpen" x-transition:enter="transition delay-150 duration-200 opacity-0" class="ms-4 text-[13px] font-bold tracking-tight whitespace-nowrap">{{ $menu['name'] }}</span>
                </a>
            @endforeach
        </nav>
        ...
        <div class="mt-auto pt-6 space-y-3 relative z-20">
            <x-dropdown align="right" width="64">
                <x-slot name="trigger">
                    <button class="w-full flex items-center gap-3 p-2 rounded-2xl bg-white/[0.03] border border-white/[0.05] hover:bg-white/[0.06] transition-all group">
                        ...
                        <div x-show="$store.sidebar.isOpen" x-transition:enter="transition delay-150 duration-200 opacity-0" class="text-left flex-1 min-w-0">
                        ...
                        <svg x-show="$store.sidebar.isOpen" class="w-3 h-3 text-slate-600 group-hover:text-indigo-400 transition-all" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M5 15l7-7 7 7" stroke-width="3"/></svg>
                    </button>
                </x-slot>
                <x-slot name="content">
            ...
            <div class="p-4 rounded-2xl bg-white/[0.02] border border-white/[0.05] text-center backdrop-blur-md shadow-inner">
                <p x-show="$store.sidebar.isOpen" x-transition:enter="transition delay-200 opacity-0" class="text-[9px] font-black text-gray-600 uppercase mb-1 tracking-widest">Status</p>
                <p class="text-[10px] font-bold text-indigo-400" x-text="$store.sidebar.isOpen ? 'GOLD EDITION V.2.1' : 'V.2.1'"></p>
            </div>
        </div>
    </aside>

    <style>
        /* Membersihkan Style custom agar tidak konflik dengan Z-Index Tailwind */
