<div class="flex h-screen w-full bg-[#080808] text-slate-300 overflow-hidden" 
    ...
    <div x-show="isMobileOpen" 
         ...
         class="fixed inset-0 z-[60] bg-black/80 backdrop-blur-md md:hidden">
    </div>
    ...
    <aside 
        ...
        class="fixed inset-y-0 left-0 z-[70] flex flex-col bg-[#050505] border-r border-white/[0.05] transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] md:relative md:translate-x-0">
        
        <div class="h-20 flex items-center px-6 shrink-0 border-b border-white/[0.03] relative overflow-hidden">
            ...
            <a href="{{ route('dashboard') }}" class="flex items-center gap-4 group relative z-10 min-w-0">
                <div class="shrink-0">
                    <div class="relative bg-gradient-to-br from-indigo-600/20 to-purple-600/20 border border-white/10 p-2 rounded-xl backdrop-blur-md group-hover:border-indigo-500/50 transition-colors">
                         <x-application-logo class="h-6 w-auto text-white" />
                    </div>
                </div>
                <div x-show="!isCollapsed || isMobileOpen" x-transition:enter="delay-150 duration-300" class="min-w-0">
        ...
        <nav class="flex-1 overflow-y-auto py-2 px-3 space-y-1 custom-scroll relative z-10 overscroll-contain">
            @php
                $menus = [
                    [
                        'name' => 'User Management', 
                        'route' => 'admin.users.index', 
                        'icon' => 'M18 18.72a6 6 0 0 0-3.44-5.15m2.07-3.57a3 3 0 1 0-4.26-4.26m5.63 7.83a7 7 0 1 1-9.42-9.42m0 12.15a6 6 0 0 1-3.44-5.15m2.07-3.57a3 3 0 1 1-4.26-4.26m5.63 7.83a7 7 0 1 0-9.42-9.42'
                    ], 
                    [
                        'name' => 'Kapster', 
                        'route' => 'admin.kapsters.index', 
                        'icon' => 'M4 19a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2H4ZM14 5l-2 5M10 5l2 5M12 10l-4 9M12 10l4 9'
                    ], 
                    [
                        'name' => 'Kapster Shifts', 
                        'route' => 'admin.kapster_shifts.index', 
                    ...
                    [
                        ...
                        'icon' => 'M19 4H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2Zm-7 15l-5-5 1.41-1.41L12 16.17l7.59-7.59L21 10l-9 9Z'
                    ],
                    [
            ...
            @foreach($menus as $menu)
                ...
                <a href="{{ route($menu['route']) }}" 
                   class="group flex items-center p-3 rounded-xl transition-all duration-300 {{ $isActive ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'text-gray-500 hover:bg-white/[0.04] hover:text-white' }}">
                    ...
                    <span x-show="!isCollapsed || isMobileOpen" class="ms-4 text-[13px] font-bold tracking-tight whitespace-nowrap">{{ $menu['name'] }}</span>
                </a>
            @endforeach
                    ...
                    <x-slot name="trigger">
                        <button class="w-full flex items-center justify-between p-2 rounded-xl hover:bg-white/5 transition-all group outline-none">
                            <div class="flex items-center gap-3 min-w-0">
                                <div class="shrink-0 w-10 h-10 rounded-lg bg-gradient-to-tr from-indigo-600 to-purple-600 flex items-center justify-center text-white font-black text-sm shadow-lg group-hover:scale-95 transition-transform">
                            ...
                            <div x-show="!isCollapsed || isMobileOpen" class="text-slate-600 group-hover:text-indigo-400">
        ...
        <div class="mt-auto p-4 bg-black/40">
    ...
    <div class="flex flex-col flex-1 min-w-0 overflow-hidden relative">
        <header class="h-20 flex items-center py-4 justify-between px-4 md:px-8 bg-[#050505]/50 backdrop-blur-xl border-b border-white/[0.05] sticky top-0 z-40">
            <div class="flex items-center gap-3">
                <button @click="isMobileOpen = true" 
                        class="md:hidden p-2.5 rounded-xl bg-white/5 text-white active:scale-95 transition-all">
        ...
        <main class="flex-1 overflow-y-auto custom-scroll p-4 md:p-8 lg:p-12 relative">
