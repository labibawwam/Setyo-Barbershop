    ...
    <div class="flex flex-col flex-1 min-w-0 overflow-hidden relative">
        ...
        <header class="h-20 flex items-center py-4 justify-between px-4 md:px-8 bg-white/80 backdrop-blur-xl border-b border-slate-200 sticky top-0 z-50">
            <div class="flex items-center gap-4">
                <button @click="isMobileOpen = true" class="md:hidden p-2.5 rounded-xl bg-white border border-slate-200 text-slate-600 active:scale-95 transition-all shadow-sm">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16m-7 6h7" />
                    </svg>
                </button>

                <button @click="isCollapsed = !isCollapsed" class="hidden md:flex p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-indigo-600 hover:bg-indigo-600 hover:text-white transition-all shadow-sm">
