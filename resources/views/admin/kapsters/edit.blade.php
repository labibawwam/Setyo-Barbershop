<x-app-layout>
    <div class="flex h-screen w-screen overflow-hidden bg-[#f8fafc] font-sans text-slate-600">
        ...
        <main class="flex-1 flex flex-col items-center relative overflow-y-auto custom-scroll bg-[#f8fafc] px-6 md:px-12 py-10 md:py-16 lg:py-20">
                    ...
                    <form action="{{ route('admin.kapsters.update', $kapster->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5 md:space-y-6">
                        ...
                        <div class="group space-y-2">
                            ...
                            <textarea 
                                ...
                                class="w-full bg-[#f8fafc] border border-slate-200 rounded-xl md:rounded-[1.5rem] px-5 md:px-6 py-3.5 md:py-4 text-sm text-slate-900 font-medium placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/5 transition-all duration-300 resize-none min-h-[100px]"
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="flex flex-col sm:flex-row items-center gap-6 md:gap-8">
                                @if($kapster->photo)
                                    <div class="shrink-0 relative group/img">
                                        <img src="{{ asset('storage/' . $kapster->photo) }}?v={{ $kapster->updated_at->timestamp }}"
                                             class="w-20 h-20 md:w-24 md:h-24 rounded-2xl object-cover border-2 border-indigo-100 shadow-md group-hover/img:border-indigo-500 transition-all duration-500">
                                        <div class="absolute inset-0 rounded-2xl bg-indigo-600/5 opacity-0 group-hover/img:opacity-100 transition-opacity pointer-events-none"></div>
                                    </div>
                                @endif
                                ...
                                <div class="relative group/upload w-full flex-1">
                                    ...
                                    >
                                    <div class="w-full bg-[#f8fafc] border-2 border-dashed border-slate-200 rounded-xl md:rounded-[1.5rem] py-6 flex flex-col items-center justify-center group-hover/upload:bg-white group-hover/upload:border-indigo-300 transition-all duration-500">
