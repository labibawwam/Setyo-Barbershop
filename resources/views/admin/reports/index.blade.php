<x-app-layout>
            ...
            <main class="flex-1 flex flex-col min-w-0 bg-[#f8fafc] relative overflow-y-auto custom-scroll">
                ...
                <div class="px-4 md:px-8 lg:px-12 py-6 md:py-10 relative z-10">
                        ...
                        <form action="{{ route('admin.reports.index') }}" method="GET" class="w-full xl:w-auto flex flex-col sm:flex-row items-stretch sm:items-center gap-3 bg-white p-2 rounded-2xl border border-slate-200 shadow-sm transition-all hover:shadow-md">
                            <div class="flex flex-1 items-center bg-slate-50/50 rounded-xl px-2 divide-x divide-slate-200">
                                ...
                                <div class="relative min-w-[110px] md:min-w-[130px] flex-1 group">
                                    ...
                                    <div class="absolute right-1 top-1/2 -translate-y-1/2 pointer-events-none text-slate-300 group-hover:text-indigo-500"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M19 9l-7 7-7-7"/></svg></div>
                                </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8 mb-8 md:mb-12">
                        <div class="lg:col-span-2 bg-white p-6 md:p-8 lg:p-10 rounded-[2rem] md:rounded-[3rem] border border-slate-200 shadow-sm relative overflow-hidden group">
                            <div class="absolute top-0 right-0 -mt-4 -mr-4 w-32 h-32 bg-indigo-500/5 blur-3xl rounded-full"></div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8 relative z-10">
                            ...
                            <div class="space-y-5">
                                ...
                                <div class="flex items-center justify-between group">
                                    <div class="flex items-center gap-3 md:gap-4">
                                        <div class="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-indigo-50 border border-indigo-100 flex items-center justify-center font-black text-indigo-600 transition-all group-hover:bg-indigo-600 group-hover:text-white">
                        ...
                        <div class="bg-white p-6 md:p-8 lg:p-10 rounded-[2rem] md:rounded-[3rem] border border-slate-200 shadow-sm">
                            <h3 class="text-[10px] md:text-xs font-black text-slate-400 uppercase tracking-widest mb-8 text-left">Layanan Terlaris</h3>
                            <div class="space-y-6">
                                ...
                                <div class="group text-left">
                                    ...
                                    <div class="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                                        ...
                                        <div class="h-full bg-gradient-to-r from-indigo-500 to-purple-500 transition-all duration-1000" style="width: {{ $percentage }}%"></div>
                                    </div>
                                </div>
                                @empty
                                <p class="text-center text-[10px] text-slate-400 py-10 uppercase italic">No data collected</p>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Use server-provided daily revenue data (keys: day -> total)
            const dailyRevenueMap = @json($dailyRevenueData ?? []);
            const dailyData = dailyLabels.map(label => {
                const asNumberKey = Number(label);
                const value = (dailyRevenueMap[label] ?? dailyRevenueMap[asNumberKey] ?? 0);
            ...
            if (ctxMain) {
                new Chart(ctxMain.getContext('2d'), {
                    ...
                    options: {
                        ...
                        scales: {
                            y: { grid: { color: '#f1f5f9', drawBorder: false }, ticks: { font: { size: 10 }, callback: (v) => v >= 1000 ? (v/1000) + 'k' : v } },
            ...
            if (ctxStatus) {
                new Chart(ctxStatus.getContext('2d'), {
                    ...
                    options: {
                        ...
                        cutout: '75%',
                        plugins: {
                            legend: { position: 'bottom', labels: { boxWidth: 6, usePointStyle: true, padding: 15, font: { size: 10, weight: '700' } } }
                        }
                    }
                });
            }
