        ...
        <main class="flex-1 flex flex-col min-w-0 bg-[#050505] relative">
                ...
                <div class="flex items-center justify-between gap-6">
                    <div class="min-w-0">
                        <h1 class="text-3xl font-black text-white tracking-tight flex items-center gap-3">
                            Barber <span class="text-indigo-500 italic font-serif text-2xl font-normal lowercase tracking-normal">services</span>
                        </h1>
                        <p class="text-[10px] font-bold uppercase tracking-[0.3em] text-slate-500 mt-2 flex items-center gap-2">
                            <span class="w-2 h-2 rounded-full bg-indigo-500 animate-pulse"></span>
                            Service Menu & Treatment Catalog
                        </p>
                    </div>
                    ...
                    <table class="w-full text-left border-separate border-spacing-y-3">
                        <thead class="sticky top-0 z-20 bg-[#050505]">
                            <tr class="text-[10px] font-black text-slate-500 uppercase tracking-[0.25em]">
                                ...
                                <th class="py-4 px-6 border-b border-white/[0.05] w-[25%]">Service Details</th>
                                <th class="py-4 px-6 border-b border-white/[0.05] w-[15%] text-center">Category</th>
                                <th class="py-4 px-6 border-b border-white/[0.05] w-[15%] text-center">Duration</th>
                                <th class="py-4 px-6 border-b border-white/[0.05] w-[20%] text-center">Premium Price</th>
                        ...
                        <tbody class="divide-y divide-transparent">
                                ...
                                <td class="py-4 px-6 bg-white/[0.03] group-hover:bg-white/[0.06] border-y border-white/[0.05]">
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div class="min-w-0">
                                            ...
                                            <div class="text-[10px] text-slate-500 font-medium truncate mt-0.5 max-w-[180px] italic">
                                                {{ $service->deskripsi }}
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <td class="py-4 px-6 bg-white/[0.03] group-hover:bg-white/[0.06] border-y border-r border-white/[0.05] rounded-r-2xl text-right">
                                    <div class="flex justify-end items-center gap-3">
                                        <a href="{{ route('admin.services.edit', $service->id) }}" 
                                           class="w-8 h-8 flex items-center justify-center rounded-lg bg-indigo-600 text-white hover:bg-indigo-500 transition-all duration-300 group/edit shadow-lg shadow-indigo-600/20 active:scale-90" title="Edit Service">
                                        ...
                                        <form action="{{ route('admin.services.destroy', $service->id) }}" method="POST" class="inline m-0" onsubmit="return confirm('Hapus layanan ini?')">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="w-8 h-8 flex items-center justify-center rounded-lg bg-red-600 text-white hover:bg-red-500 transition-all duration-300 group/del shadow-lg shadow-red-600/20 active:scale-90" title="Delete Service">
                            ...
                            @empty
                            <tr>
                                <td colspan="6" class="py-12 text-center bg-white/[0.02] rounded-2xl border border-white/5">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <p class="text-xs font-bold uppercase tracking-widest text-slate-600">No services cataloged yet</p>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
            ...
            <div class="flex-none px-8 py-6 lg:px-10 border-t border-white/[0.05] bg-[#050505]/80 backdrop-blur-md">
                <div class="flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-slate-500">
                    <p>Catalog Index: <span class="text-indigo-400 ml-1 italic font-serif text-sm font-normal">{{ $services->count() }} Treatments</span></p>
                    <div class="flex items-center gap-4">
