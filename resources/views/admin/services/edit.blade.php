            ...
            <div class="w-full max-w-2xl relative z-10 flex flex-col items-center my-auto entrance-animation">
                ...
                <div class="mb-8 md:mb-10 text-center shrink-0">
                    <h1 class="font-display text-3xl md:text-5xl font-bold text-white tracking-tight leading-tight uppercase">
                        Update <span class="text-indigo-500 italic font-serif lowercase">Service Detail</span>
                    </h1>
                    ...
                    <form action="{{ route('admin.services.update', $service->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5 md:space-y-6">
                                ...
                                <div class="relative">
                                    ...
                                    >
                                        @foreach($categories as $cat)
                                            <option value="{{ $cat->id }}" class="bg-[#0b0b0b]" {{ old('category_id', $service->category_id) == $cat->id ? 'selected' : '' }}>
                                                {{ $cat->nama_kategori }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="space-y-2 group">
                            ...
                            <textarea 
                                name="deskripsi"
                                rows="3"
                                class="w-full bg-white/[0.03] border border-white/10 rounded-xl md:rounded-2xl px-5 py-3 md:py-3.5 text-sm text-white focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all resize-none min-h-[100px]"
                                required
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="flex flex-col sm:flex-row items-center gap-6 bg-white/[0.01] p-4 rounded-2xl border border-white/5 group/media transition-all hover:bg-white/[0.03]">
                                @if($service->gambar)
                                <div class="shrink-0 relative">
                                    <img src="{{ asset('storage/' . $service->gambar) }}?v={{ $service->updated_at->timestamp }}"
                                         class="w-20 h-20 md:w-24 md:h-24 rounded-2xl object-cover border-2 border-indigo-500/30 shadow-lg group-hover/media:border-indigo-500 transition-all duration-500">
                                </div>
                                @endif
                                ...
                                <div class="relative flex-1 group/upload w-full">
                                    ...
                                    >
                                    <div class="w-full border-2 border-dashed border-white/10 rounded-xl py-6 flex flex-col items-center justify-center group-hover/upload:border-indigo-500/40 transition-all duration-500 text-center px-4">
                                        ...
                                        <span class="file-name text-[9px] font-bold text-slate-500 uppercase tracking-widest">
                        ...
                        <div class="flex flex-col sm:flex-row items-center gap-4 pt-4">
                            <button 
                                type="submit"
                                class="w-full sm:flex-[2] group relative px-8 py-3.5 md:py-4 bg-white text-black rounded-xl md:rounded-2xl text-[10px] md:text-[11px] font-black uppercase tracking-widest transition-all hover:bg-indigo-600 hover:text-white active:scale-95 overflow-hidden shadow-lg"
                            >
