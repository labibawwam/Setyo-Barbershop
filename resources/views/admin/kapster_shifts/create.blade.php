<x-app-layout>
    <div class="flex h-screen w-screen overflow-hidden bg-[#f8fafc] font-sans text-slate-600">
        ...
        <x-sidebar> 
            <main class="flex-1 flex flex-col items-center relative overflow-y-auto custom-scroll bg-[#f8fafc] px-6 md:px-12 py-10 md:py-16">
                ...
                <div class="absolute bottom-0 left-0 w-[300px] md:w-[400px] h-[300px] md:h-[400px] bg-purple-500/[0.03] blur-[70px] md:blur-[100px] pointer-events-none"></div>
                        ...
                        <form action="{{ route('admin.kapster_shifts.store') }}" method="POST" class="space-y-5 md:space-y-6">
                                    ...
                                    <div class="relative">
                                        <select name="kapster_id" class="w-full bg-[#f8fafc] border border-slate-200 rounded-xl md:rounded-2xl px-5 py-3.5 text-sm text-slate-900 font-bold appearance-none focus:outline-none focus:border-indigo-500 transition-all cursor-pointer pr-12" required>
                                            @foreach($kapsters as $kapster)
                                                <option value="{{ $kapster->id }}" class="bg-white">{{ $kapster->nama }}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="relative">
                                        <select name="hari" class="w-full bg-[#f8fafc] border border-slate-200 rounded-xl md:rounded-2xl px-5 py-3.5 text-sm text-slate-900 font-bold appearance-none focus:outline-none focus:border-indigo-500 transition-all cursor-pointer pr-12" required>
                                            @foreach(['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu', 'Minggu'] as $day)
                                                <option value="{{ $day }}" class="bg-white">{{ $day }}</option>
                                            @endforeach
                                        </select>
                                        <div class="absolute right-5 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M19 9l-7 7-7-7" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/></svg>
                            ...
                            <div id="time-inputs" class="grid grid-cols-1 sm:grid-cols-2 gap-5 md:gap-6 transition-all duration-500">
                                ...
                                <div class="space-y-2 group sm:col-span-2">
                                    <label class="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1 group-focus-within:text-indigo-600 transition-colors">Interval Slot</label>
                                    <select name="slot_interval" class="mt-2 w-40 bg-[#f8fafc] border border-slate-200 rounded-xl px-4 py-2 text-sm text-slate-900">
                                        @foreach([15,20,25,30,35,40,45,50,55,60] as $i)
                                            <option value="{{ $i }}" {{ $i == 30 ? 'selected' : '' }}>{{ $i }} menit</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
    ...
    <style>
        ...
        input[type="time"]::-webkit-calendar-picker-indicator { filter: none; opacity: 0.6; cursor: pointer; }
