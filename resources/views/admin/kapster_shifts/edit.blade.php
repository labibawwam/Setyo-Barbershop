<x-app-layout>
    <div class="flex h-screen w-screen overflow-hidden bg-[#050505] font-sans text-slate-300">
        ...
        <main class="flex-1 flex flex-col items-center relative overflow-y-auto custom-scroll bg-[#050505] px-6 md:px-12 py-10 md:py-16">
            ...
            <div class="absolute top-0 right-0 w-[300px] md:w-[600px] h-[300px] md:h-[600px] bg-purple-600/5 blur-[80px] md:blur-[120px] pointer-events-none"></div>
            <div class="absolute bottom-0 left-0 w-[200px] md:w-[400px] h-[200px] md:h-[400px] bg-indigo-600/5 blur-[70px] md:blur-[100px] pointer-events-none"></div>
            ...
            <div class="w-full max-w-2xl relative z-10 flex flex-col items-center my-auto entrance-animation">
                ...
                <div class="mb-8 md:mb-10 text-center shrink-0">
                    <h1 class="font-display text-3xl md:text-5xl font-bold text-white tracking-tight leading-tight uppercase">
                        Modify <span class="text-indigo-500 italic font-serif lowercase tracking-normal">Shift Schedule</span>
                    </h1>
                    <p class="text-[8px] md:text-[10px] font-bold uppercase tracking-[0.3em] md:tracking-[0.4em] text-slate-500 mt-3 flex items-center justify-center gap-2">
                ...
                <div class="w-full bg-white/[0.02] border border-white/[0.05] backdrop-blur-3xl rounded-[2.5rem] md:rounded-[3rem] p-6 md:p-10 lg:p-12 shadow-[0_25px_80px_rgba(0,0,0,0.7)]">
                    <form action="{{ route('admin.kapster_shifts.update', $kapsterShift->id) }}" method="POST" class="space-y-5 md:space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="space-y-2 group">
                            ...
                            <div class="relative">
                                <select name="hari" class="w-full bg-white/[0.03] border border-white/10 rounded-xl md:rounded-2xl px-5 py-3.5 md:py-4 text-sm text-white appearance-none focus:outline-none focus:border-indigo-500 transition-all cursor-pointer pr-12" required>
                        ...
                        <div id="time-inputs" class="grid grid-cols-1 sm:grid-cols-2 gap-5 md:gap-6 transition-all duration-500 {{ $kapsterShift->is_libur ? 'opacity-10 pointer-events-none grayscale' : '' }}">
                            <div class="space-y-2 group">
                                ...
                                <input type="time" name="jam_mulai" value="{{ \Carbon\Carbon::parse($kapsterShift->jam_mulai)->format('H:i') }}" class="w-full bg-white/[0.03] border border-white/10 rounded-xl md:rounded-2xl px-5 py-3.5 md:py-4 text-sm text-white focus:outline-none focus:border-indigo-500 transition-all [color-scheme:dark]">
                            </div>
                            <div class="space-y-2 group">
                                ...
                                <input type="time" name="jam_selesai" value="{{ \Carbon\Carbon::parse($kapsterShift->jam_selesai)->format('H:i') }}" class="w-full bg-white/[0.03] border border-white/10 rounded-xl md:rounded-2xl px-5 py-3.5 md:py-4 text-sm text-white focus:outline-none focus:border-indigo-500 transition-all [color-scheme:dark]">
                            </div>
                        </div>

                        <div class="flex items-center gap-3 px-1 py-1">
                            <input type="checkbox" name="is_libur" id="is_libur" class="w-5 h-5 rounded border-white/10 bg-white/5 text-indigo-600 focus:ring-indigo-500 transition-all cursor-pointer" {{ $kapsterShift->is_libur ? 'checked' : '' }} onchange="toggleTimeInputs(this)">
