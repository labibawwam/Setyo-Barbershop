<x-app-layout>
    <div class="flex h-screen w-screen overflow-hidden bg-[#f8fafc] font-sans text-slate-600">
        ...
        <main class="flex-1 flex flex-col min-w-0 bg-[#f8fafc] relative overflow-hidden">
            ...
            <div class="absolute bottom-0 left-1/4 w-[200px] md:w-[400px] h-[200px] md:h-[400px] bg-purple-500/[0.03] blur-[70px] md:blur-[100px] pointer-events-none"></div>
            
            <div class="flex-none px-6 md:px-8 py-6 md:py-8 lg:px-12 border-b border-slate-200 bg-white/70 backdrop-blur-xl z-20">
                    ...
                    <table class="w-full text-left border-separate border-spacing-y-3 min-w-[900px] lg:min-w-full">
                        ...
                        <tbody class="divide-y divide-transparent">
                            ...
                            <tr class="group transition-all duration-500">
                                <td class="py-5 px-4 bg-white group-hover:bg-slate-50 rounded-l-2xl border-y border-l border-slate-200 text-center font-mono text-xs text-slate-400 transition-all shadow-sm">
                                    {{ str_pad($loop->iteration, 2, '0', STR_PAD_LEFT) }}
                                </td>

                                <td class="py-5 px-6 bg-white group-hover:bg-slate-50 border-y border-slate-200 transition-all shadow-sm">
                                    <div class="flex items-center gap-4">
                                        <div class="shrink-0 w-12 h-12 rounded-2xl bg-indigo-50 border border-indigo-100 flex items-center justify-center text-sm font-black text-indigo-600 shadow-inner group-hover:scale-105 transition-all">
                                            {{ strtoupper(substr($user->name, 0, 1)) }}
                                        </div>
                                        <div class="min-w-0">
                                            <div class="text-sm font-bold text-slate-800 group-hover:text-indigo-600 transition-colors truncate">{{ $user->name }}</div>
                                            <div class="text-[11px] text-slate-500 font-medium truncate italic tracking-wide mt-0.5">{{ $user->email }}</div>
                                        </div>
                                    </div>
                                </td>

                                <td class="py-5 px-6 bg-white group-hover:bg-slate-50 border-y border-slate-200 text-center transition-all shadow-sm">
                                    <span class="inline-flex items-center px-4 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-[0.15em] {{ $user->role == 'admin' ? 'bg-indigo-600 text-white shadow-md shadow-indigo-100' : 'bg-slate-100 text-slate-600 border border-slate-200' }}">
                                        {{ $user->role }}
                                    </span>
                                </td>

                                <td class="py-5 px-6 bg-white group-hover:bg-slate-50 border-y border-r border-slate-200 rounded-r-2xl text-right transition-all shadow-sm">
                                    <div class="flex justify-end items-center gap-3">
                                        <a href="{{ route('admin.users.edit', $user->id) }}" class="w-10 h-10 flex items-center justify-center rounded-xl bg-indigo-50 border border-indigo-100 text-indigo-600 hover:bg-indigo-600 hover:text-white transition-all duration-300 shadow-sm">
