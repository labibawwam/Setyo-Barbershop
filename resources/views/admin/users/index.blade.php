<x-app-layout>
    <div class="flex h-screen w-screen overflow-hidden bg-[#050505] font-sans text-slate-300">
        ...
        <main class="flex-1 flex flex-col min-w-0 bg-[#050505] relative">
            ...
            <div class="flex-none px-8 py-8 lg:px-12 border-b border-white/[0.05] bg-[#050505]/50 backdrop-blur-md z-20">
                <div class="flex items-center justify-between gap-6">
                    <div class="min-w-0">
                        <h1 class="text-3xl md:text-4xl font-black text-white tracking-tight flex items-center gap-3">
                            Users <span class="text-indigo-500 italic font-serif text-2xl font-normal lowercase tracking-normal">directory</span>
                        </h1>
                        <p class="text-[10px] font-bold uppercase tracking-[0.3em] text-slate-500 mt-2 flex items-center gap-2">
                            <span class="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
                            Verified Database Management
                        </p>
                    </div>
                    
                    <a href="{{ route('admin.users.create') }}" class="shrink-0 group relative px-8 py-3.5 bg-white text-black rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all hover:bg-indigo-600 hover:text-white active:scale-95 overflow-hidden shadow-xl">
            ...
            <div class="flex-1 overflow-hidden flex flex-col relative z-10">
                <div class="flex-1 overflow-y-auto overflow-x-auto custom-scroll px-8 lg:px-12 py-6">
                    <table class="w-full text-left border-separate border-spacing-y-3">
                        <thead class="sticky top-0 z-30 bg-[#050505]">
                            <tr class="text-[10px] font-black text-slate-500 uppercase tracking-[0.25em]">
                                ...
                                <th class="py-4 px-6 border-b border-white/[0.05] text-right w-[180px]">System Actions</th>
                            </tr>
                        </thead>
                            ...
                            <tr class="group transition-all duration-500">
                                ...
                                <td class="py-5 px-6 bg-white/[0.02] group-hover:bg-white/[0.05] border-y border-r border-white/[0.05] rounded-r-2xl text-right transition-all">
                                    <div class="flex justify-end items-center gap-3">
                                        <a href="{{ route('admin.users.edit', $user->id) }}" class="w-10 h-10 flex items-center justify-center rounded-xl bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 hover:bg-indigo-600 hover:text-white transition-all duration-300 shadow-sm" title="Edit Profile">
                                        ...
                                        <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST" class="inline m-0" onsubmit="return confirm('Secure Warning: Are you sure you want to delete this user?');">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="w-10 h-10 flex items-center justify-center rounded-xl bg-red-500/10 border border-red-500/20 text-red-500 hover:bg-red-600 hover:text-white transition-all duration-300 shadow-sm" title="Delete Entry">
            ...
            <div class="flex-none px-8 py-6 lg:px-12 border-t border-white/[0.05] bg-[#050505]/80 backdrop-blur-md">
                <div class="flex justify-between items-center text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">
                    <p>Total Database Capacity: <span class="text-indigo-400 italic font-serif text-sm font-normal normal-case tracking-normal ml-2">{{ $users->count() }} Registered Accounts</span></p>
